<x-slot name="breadcrumbs">
    {{ Breadcrumbs::render('report.delivery') }}
</x-slot>
...
<section>
    ...
    <div style='border-bottom: 2px solid #eaeaea' class="mb-5">
        <ul class='flex cursor-pointer'>
          <li class='py-2 px-6 bg-white rounded-t-lg'><a href="{{ route('report.delivery') }}">Livraisons</a></li>
          <li class='py-2 px-6 rounded-t-lg text-gray-500 bg-gray-200'><a href="{{ route('report.request') }}">Demandes</a></li>
        </ul>
    </div>

    <x-section-header title="Rapport des livraisons">
        <x-slot name="actions">
            <x-button-link class="btn-white" target="_blank" href="{{ route('report.delivery.pdf', $filters + ['download' => true]) }}">
                Telecharger le PDF
            </x-button-link>
        </x-slot>
    </x-section-header>
        ...
        <div class="grid gap-8 md:grid-cols-12">
            <div class="col-span-8 md:col-span-4">
                ...
                <div class="flex items-center space-x-2 justify-between">
                    ...
                    <div>
                        <x-loading-button class="btn-primary text-gray-50 mt-0 md:mt-8 w-full" type="submit">
                            Appliquer
                        </x-loading-button>
                    </div>
                </div>
            ...
            <div class="col-span-8 relative">
                ...
                <iframe id="delivery-iframe" class="w-full h-screen border-gray-100 border-solid rounded md:flex" src="{{ route('report.delivery.pdf', $filters) }}"></iframe>
            </div>
        </div>
    </form>
